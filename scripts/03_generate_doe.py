...
def generate_synthetic_data(
    n_samples: int = 200, output_path: str = "membrane_doe.csv"
):
    ...
    for i in range(n_samples):
        try:
            ...
            raise RuntimeError(
                f"Unexpected critical failure at iteration {i}: {str(e)}"
            ) from e
    ...
    print(
        f"DOE Data saved to {output_path}. Successful cases: {n_samples - failures}/{n_samples}"
    )
