...
class MembraneReactor1D:
    ...
    def __init__(
        ...
        if not all(
            ...
            for val in [length, diameter, rho_cat, void_frac]
        ):
            raise TypeError(
                "All reactor physical parameters must be numeric (int or float)."
            )
    ...
    def vbf_kinetics(self, p: np.ndarray, T: float) -> Tuple[float, float]:
        ...
        if T <= 0:
            raise ValueError(
                "Absolute temperature (T) must be strictly positive (Kelvin)."
            )
    ...
    def simulate(
        ...
        if flow_in <= 0 or P_in <= 0 or h2_co2_ratio <= 0:
            raise ValueError(
                "Operational parameters (flow, pressure, ratio) must be strictly positive."
            )
        ...
        def odefunc(z: float, F: np.ndarray) -> np.ndarray:
            ...
            stoich = np.array([[-1, -3, 0, 1, 1], [-1, -1, 1, 0, 1]])
        ...
        if not sol.success:
            raise ODENonConvergenceError(
                f"ODE Solver failed to converge at z={sol.t[-1]:.4f}: {sol.message}"
            )
