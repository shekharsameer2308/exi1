...
def generate_diagnostic_dashboard(output_file: str = "reports/figures/reactor_intensification_dashboard.png"):
    ...
    ax1 = axs[0, 0]
    ax1.plot(z * 1000.0, yield_mr, color=c_emerald, linewidth=2.4, label="Intensified CMR (NaA / OM/Vr = 133.3 m^-1)")
    ax1.plot(z * 1000.0, yield_tr, color=c_slate, linestyle="--", linewidth=2.0, label="Conventional Fixed-Bed (TR Ceiling)")
    ax1.axhline(25.71, color=c_red, linestyle=":", label="TR Equilibrium Boundary (25.71%)")
    ax1.fill_between(z * 1000.0, yield_tr, yield_mr, color=c_emerald, alpha=0.12, label="Equilibrium Shift Gain (+14.77%)")
    ...
    ax2 = axs[0, 1]
    ax2.plot(z * 1000.0, p_h2o_mr, color=c_blue, linewidth=2.4, label="Retentate Bed p_H2O(z)")
    ax2.plot(z * 1000.0, p_h2o_crit, color=c_red, linestyle="-.", linewidth=2.2, label="Critical Boundary (1.5% * P_tot = 1.50 bar)")
    ax2.fill_between(z * 1000.0, 0.0, p_h2o_crit, color=c_red, alpha=0.15, label="Deactivation Zone (alpha-CuZn Formation)")
    ax2.fill_between(z * 1000.0, p_h2o_crit, 8.0, color=c_emerald, alpha=0.08, label="Catalyst Preservation Envelope")
    ...
    ax3 = axs[1, 0]
    ax3.axhspan(1.0, 1.5, color=c_emerald, alpha=0.18, label="Optimal Intensification (1.0 <= theta_m <= 1.5)")
    ax3.axhspan(0.0, 1.0, color=c_amber, alpha=0.12, label="Kinetics Limited (theta_m < 1.0)")
    ax3.axhspan(1.5, 3.0, color=c_red, alpha=0.12, label="Over-Extraction Risk (theta_m > 1.5)")
    ...
    ax3.scatter([1.8], [1.25], color=c_blue, s=150, marker="*", zorder=6, label="Hauth et al. Base Case (theta_m=1.25)")
    ...
    ax4.scatter([1.18], [0.82], color=c_red, s=140, marker="D", edgecolors="black", label="Base Design (STY=1.18, 0.82 kWh/kg)")
