...
from core.thermodynamics import (
    ...
    get_mixture_heat_capacity, get_reaction_enthalpies, peng_robinson_eos
)
...
class AnnularReactor2DSolver:
    ...
    def solve(self) -> SimulationResults:
        ...
        rem_H2O = float(0.0 if not is_mem else (1.0 - (F_out[3] / total_h2o_gen)) * 100.0)
        
        p_final_bar = (F_out / np.sum(F_out)) * (P_ret / 1.0e5)
        p_h2o_exit_bar = float(p_final_bar[3])
        
        is_safe, margin, safe_msg = check_catalyst_stability(p_h2o_exit_bar, P_ret / 1e5)
