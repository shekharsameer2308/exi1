...
from core.thermodynamics import (
    SPECIES, SPECIES_IDX, N_SPECIES, MOLAR_MASSES, R_GAS,
    get_mixture_heat_capacity, get_reaction_enthalpies, peng_robinson_compressibility
)
from core.kinetics import calculate_reaction_rates, check_catalyst_preservation
from core.transport_2d import (
    compute_radial_dispersion, compute_effective_radial_conductivity,
    compute_wall_heat_transfer_coefficient, update_dynamic_ergun
)
...
class ReactorParameters:
    ...
    U_wall_fixed: Optional[float] = None
    ...
    separation_factors: Dict[str, float] = field(default_factory=lambda: {
        "H2O/H2": 220.0,
        "H2O/CO2": 450.0,
        "H2O/CH3OH": 600.0,
        "H2O/CO": 500.0,
...
class ReactorEngine2D:
    ...
    def solve(self) -> Simulation2DResult:
        ...
        Z_in, rho_in = peng_robinson_compressibility(self.y_in, self.p.T_in_K, self.p.P_in_Pa)
        u_s_in = (self.F_feed_total_mol_s * Z_in * R_GAS * self.p.T_in_K) / (self.p.P_in_Pa * self.A_c)
        ...
        def ode_rhs(z: float, state: np.ndarray) -> np.ndarray:
            ...
            F_ret_total_est = np.sum(np.dot(C_grid, self.r_weights)) * u_s_in * self.A_c # approximate scale
            ...
            k_er = compute_effective_radial_conductivity(
                u_s=u_s, rho_g=rho_g, Cp_g=Cp_mix, d_p=self.p.d_p_m, eps=self.p.bed_voidage
            )
            U_wall = self.p.U_wall_fixed if self.p.U_wall_fixed is not None else compute_wall_heat_transfer_coefficient(
                u_s=u_s, rho_g=rho_g, mu_g=1.5e-5, Cp_g=Cp_mix, k_g=0.04, d_p=self.p.d_p_m, d_t=self.p.diameter_inner_m
            )
            ...
            Q_h2o_T = self.p.water_permeance_mol_m2_s_Pa * np.exp(
                -(self.p.permeance_Ea_kJ_mol * 1e3 / R_GAS) * (1.0 / T_wall - 1.0 / 503.15)
            )
            
            # Wall partial pressures [bar]
            p_wall_bar = y_grid[:, -1] * (P_loc / 1e5)
            p_perm_water_bar = (self.p.P_perm_Pa / 1e5) * self.p.sweep_water_mol_fraction
            
            # Membrane Fluxes J_i [mol / (m^2 * s)]
            # Water flux
            driving_force_water_Pa = max((p_wall_bar[SPECIES_IDX["H2O"]] - p_perm_water_bar) * 1e5, 0.0)
            ...
            J_fluxes[SPECIES_IDX["CO2"]] = J_water / self.p.separation_factors["H2O/CO2"]
            J_fluxes[SPECIES_IDX["CH3OH"]] = J_water / self.p.separation_factors["H2O/CH3OH"]
            ...
            q_desorption_sink = np.pi * d_m * J_water * (dH_des_J_mol + 35.0 * (T_wall - 483.15))
            ...
            for r_k in range(Nr):
                ...
                if r_k == 0:
                    ...
                    for sp_i in range(N_SPECIES):
                        ...
                        lap_C = (C_ghost - 2.0 * C_grid[sp_i, Nr - 1] + C_grid[sp_i, Nr - 2]) / dr2 + (1.0 / r_val) * dC_dr_wall
                        dstate[idx_C(sp_i, r_k)] = (D_er * lap_C + R_gen[sp_i]) / u_s
                        
                    # Wall thermal flux including external cooling and endothermic desorption sink
                    q_wall_total = U_wall * (T_wall - self.p.T_cool_K) + (q_desorption_sink / (np.pi * d_m))
                    dT_dr_wall = -q_wall_total / max(k_er, 1e-4)
                    T_ghost = T_grid[Nr - 2] + 2.0 * dr * dT_dr_wall
                    lap_T = (T_ghost - 2.0 * T_grid[Nr - 1] + T_grid[Nr - 2]) / dr2 + (1.0 / r_val) * dT_dr_wall
                    dstate[idx_T(r_k)] = (k_er * lap_T + Q_rxn) / (rho_g * Cp_mix * u_s)
                    
                else:
                    # Interior radial nodes
                    for sp_i in range(N_SPECIES):
                        d2C = (C_grid[sp_i, r_k + 1] - 2.0 * C_grid[sp_i, r_k] + C_grid[sp_i, r_k - 1]) / dr2
                        dC_dr = (C_grid[sp_i, r_k + 1] - C_grid[sp_i, r_k - 1]) / (2.0 * dr)
        ...
        for iz in range(n_z):
            ...
            for r_k in range(Nr):
                r_m, r_w, _ = calculate_reaction_rates(y_loc[r_k, :] * (P_axial[iz] / 1e5), T_profiles[iz, r_k])
            ...
            J_membrane[iz, SPECIES_IDX["H2O"]] = Q_w * max((p_w_h2o - p_perm_h2o) * 1e5, 0.0)
        ...
        F_perm_out_species = np.array([sol.y[idx_Fperm(sp_i), -1] for sp_i in range(N_SPECIES)])
        ...
        meoh_yield = (F_ret_out_species[SPECIES_IDX["CH3OH"]] + F_perm_out_species[SPECIES_IDX["CH3OH"]]) / F_co2_in
        ...
        total_h2o_produced = F_ret_out_species[SPECIES_IDX["H2O"]] + F_perm_out_species[SPECIES_IDX["H2O"]]
        water_extraction_ratio = F_perm_out_species[SPECIES_IDX["H2O"]] / max(total_h2o_produced, 1e-8)
        ...
        min_p_h2o_bar = float(np.min(p_h2o_axial_bar[1:]))  # Exclude exact inlet point where no reaction occurred yet
        
        # Space-Time Yield [kg_MeOH / (L_cat * h)]
        meoh_rate_kg_s = (F_ret_out_species[SPECIES_IDX["CH3OH"]] + F_perm_out_species[SPECIES_IDX["CH3OH"]]) * MOLAR_MASSES[SPECIES_IDX["CH3OH"]]
        ...
        F_unreacted_ret = F_ret_out_species[SPECIES_IDX["CO2"]] + F_ret_out_species[SPECIES_IDX["H2"]] + F_ret_out_species[SPECIES_IDX["CO"]]
        ...
        W_feed_comp_kJ_mol = (1.4 / 0.4) * R_GAS * 300.0 * (((50.0 / 30.0)**(0.4 / 1.4)) - 1.0) / (0.78 * 1000.0)
        # Recycle compression (dP ~ 2.5 bar)
        W_rec_comp_kJ_mol = (1.4 / 0.4) * R_GAS * 310.0 * (((50.0 / 47.5)**(0.4 / 1.4)) - 1.0) / (0.78 * 1000.0)
        # Permeate vacuum pump / recompression (from 1.2 bar to 1.0 bar / condenser)
        W_perm_pump_kJ_mol = (1.33 / 0.33) * R_GAS * 350.0 * (((1.2 / 1.0)**(0.33 / 1.33)) - 1.0) / (0.75 * 1000.0)
        ...
        n_C_out = (F_ret_out_species[0] + F_ret_out_species[2] + F_ret_out_species[3] +
                   F_perm_out_species[0] + F_perm_out_species[2] + F_perm_out_species[3])
        c_err = abs(n_C_in - n_C_out) / max(n_C_in, 1e-8)
        
        # Hydrogen Balance: 2*H2 + 4*CH3OH + 2*H2O
        n_H_in = 2.0 * self.F_in_species[1] + 4.0 * self.F_in_species[3] + 2.0 * self.F_in_species[4]
        n_H_out = (2.0 * F_ret_out_species[1] + 4.0 * F_ret_out_species[3] + 2.0 * F_ret_out_species[4] +
                   2.0 * F_perm_out_species[1] + 4.0 * F_perm_out_species[3] + 2.0 * F_perm_out_species[4])
        h_err = abs(n_H_in - n_H_out) / max(n_H_in, 1e-8)
        
        # Oxygen Balance: 2*CO2 + CO + CH3OH + H2O
        n_O_in = 2.0 * self.F_in_species[0] + self.F_in_species[2] + self.F_in_species[3] + self.F_in_species[4]
        n_O_out = (2.0 * F_ret_out_species[0] + F_ret_out_species[2] + F_ret_out_species[3] + F_ret_out_species[4] +
                   2.0 * F_perm_out_species[0] + F_perm_out_species[2] + F_perm_out_species[3] + F_perm_out_species[4])
        o_err = abs(n_O_in - n_O_out) / max(n_O_in, 1e-8)
        
        # Safety Evaluation
        is_safe, margin, safe_msg = check_catalyst_preservation(min_p_h2o_bar, self.p.P_in_Pa / 1e5)
