...
Uses Gaussian Process surrogate modeling and Pareto frontier generation (qEI / NSGA-II)
to optimize Space-Time Yield (STY) and Specific Energy Consumption.
"""
...
class BayesianReactorOptimizer:
    ...
    def __init__(self, n_init_samples: int = 15):
        ...
        self.gp_sty = GaussianProcessRegressor(kernel=kernel_sty, n_restarts_optimizer=5, normalize_y=True)
        self.gp_energy = GaussianProcessRegressor(kernel=kernel_energy, n_restarts_optimizer=5, normalize_y=True)
    ...
    def run_optimization(self, n_iterations: int = 20) -> pd.DataFrame:
        ...
        # 1. Initial Latin Hypercube / Uniform random design
        np.random.seed(42)
        X_init = np.zeros((self.n_init, 4))
        X_init[:, 0] = np.random.uniform(self.bounds["length_m"][0], self.bounds["length_m"][1], self.n_init)
        X_init[:, 1] = np.random.uniform(self.bounds["diameter_inner_m"][0], self.bounds["diameter_inner_m"][1], self.n_init)
        X_init[:, 2] = np.random.uniform(self.bounds["GHSV_h"][0], self.bounds["GHSV_h"][1], self.n_init)
        X_init[:, 3] = np.random.uniform(self.bounds["T_cool_C"][0], self.bounds["T_cool_C"][1], self.n_init)
        
        # Include baseline standard design as point 0
    ...
    def get_pareto_frontier(self) -> pd.DataFrame:
        ...
        # Filter points meeting physical safety constraints
        valid = df[
        ...
        # Non-dominated sorting: Maximize STY, Minimize Specific Energy
        ...
        for i in range(len(valid)):
            for j in range(len(valid)):
                if i != j:
                    if (sty[j] >= sty[i] and energy[j] <= energy[i]) and (sty[j] > sty[i] or energy[j] < energy[i]):
                        pareto_flags[i] = False
                        break
                        
        pareto_df = valid[pareto_flags].sort_values(by="STY_MeOH_kg_L_h", ascending=False)
        return pareto_df
